...
def add_transcation(receiver_id: str,
    ...
    # check receiver
    sender = secret_key
    data_receiver = db.query(Candidate).filter(
        Candidate.candidate_id == receiver_id).first()
    ...
    check_validation = db.query(models.Voter).filter(
        Voter.secret_key == sender).first()
    if check_validation:
